...
def enf(solid_mode, func, *args, **kwargs):
    """
    Returns a callable representation of enforced boundary conditions.
    Solid modes: +1 (add solid)
                  0 (passthrough)
                 -1 (remove solid where present)
    Parameters:
     - func [callable]: A boundary enforcing function that takes (sim, *args, **kwargs)
    Returns [callable](sim) with [callable].solid_mode set
    """
    l = lambda sim: func(sim, *args, **kwargs)
    l.solid_mode = solid_mode
    return l
