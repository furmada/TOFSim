...
def gs_3d(V, boundary_mask):
    ...
    """
    dV = 0
    for point, v_old in np.ndenumerate(V):
        if (point[0] == 0 or point[1] == 0 or point[2] == 0) or \
            ...
            continue
        elif boundary_mask[point] == 0: # Ignore boundary conditions when computing
            i, j, k = point
...
def gs_2d(V, boundary_mask):
    ...
    for point, v_old in np.ndenumerate(V):
        if (point[0] == 0 or point[1] == 0) or \
            ...
            # The point is on the boundary. Keep initial value.
            continue
        elif boundary_mask[point] == 0: # Ignore boundary conditions when looking for error
            i, j = point
...
class EMSimulationSpace(object):
    ...
    def load(filepath):
        ...
        space.V = data[4]
        space.solid_mask = data[5]
        space.boundary_mask = data[6]
        space.get_efield()
        return space
    ...
    def save(self, filepath):
        ...
        np.save(filepath, np.array([self.space_size, self.scale, self.top_left, self.axis_names,
                                    self.V, self.sold_mask, self.boundary_mask], dtype=object))
    ...
    def find_object_masks(self, boundary_enfs):
        """
        Computes the solid regions of the simulation space.
        Parameters:
         - boundary_enfs list([callable]): a list functions which impose boundary conditions given the simulation space.
        Produces and returns the self.boundary_mask which is 1 where object enforces boundary conditions,
        and self.solid_mask which is 1 where the object is solid.
        """
        self.solid_mask = np.zeros(self.V.shape, int)
        self.boundary_mask = np.zeros(self.V.shape, int)
        for enf in boundary_enfs:
            if enf.solid_mode != 0:
                self.V = np.full(self.V.shape, np.inf)
                mstep = np.zeros(self.V.shape, int)
                enf(self)
                np.isfinite(self.V, mstep, where=1)
                self.boundary_mask += mstep
                self.solid_mask += (mstep * enf.solid_mode)
        np.clip(self.boundary_mask, 0, 1, out=self.boundary_mask)
        np.clip(self.solid_mask, 0, 1, out=self.solid_mask)
        self.V = np.zeros(self.V.shape, float)
        return self.solid_mask, self.boundary_mask
    
    def compute(self, boundary_enfs, convergence_limit=1e-6, transient_ignore=100, maximum_iter=1e6, debug=False):
        """
        Compute the potential from boundary conditions.
        Parameters:
         - boundary_enfs list([callable]): a function which imposes boundary conditions given the simulation space.
    ...
    def detect_hit(self, position, velocity, radius=0):
        ...
        for axis, component in enumerate(velocity):
            ...
            try:
                if self.solid_mask[self.global_unit_to_point(position + step)] == 1:
                    # The particle bounces in this direction. Flip the respective component.
                    return_v[axis] = -component
...
class ChargedParticle(object):
    ...
    def compute_motion(self, t_span, stop_cond=None):
        ...
        step = (t_span[1] - t_span[0]) * 0.01
        if np.linalg.norm(self.initial_velocity) > 0:
            step = (min(self.sim.space_size) / self.sim.scale) / np.linalg.norm(self.initial_velocity)
        return solve_ivp(self.eom, t_span,
                         y0=np.ravel([self.initial_location, self.initial_velocity]),
                         method="DOP853",
                         first_step=0.01 * step, max_step=step,
                         events=stop_events)
...
class ChargedParticle3D(ChargedParticle):
    def __init__(self, sim, mass, charge, location, velocity, radius=0, gravity=-1, bounce=None, track_force=False):
        ...
        """
        radius = max(radius, 1.0 * (min(sim.space_size) / sim.scale) if sim != None else 1e-8)
    ...
    def collision_event(self, t, y):
        ...
        if hit_v.any() and np.linalg.norm(x - self.initial_location) > self.radius:
        ...
        while total_time < t_span[1]:
            try:
                ...
                if not self.bounce_velocity is None:
                    ...
                    if self.bounce == 0:
                        # The particle has ceased moving
                        break
                    else:
    ...
    @staticmethod
    def generate_particles(n, sim, m_avg, q_avg, l0_avg, v0_avg, r_avg,
                            m_std=0, q_std=0, l0_std=(0, 0, 0), v0_std=(0, 0, 0), r_std=0,
        ...
        Parameters:
         ...
         - r_avg... r_std: the radius average and st. dev.
        ...
        for _ in range(n):
            ...
            rad = np.random.normal(loc=r_avg, scale=r_std)
            particles.append(ChargedParticle3D(sim, mass, charge, loc, vel, radius=rad, bounce=bounce_coef, track_force=track_f))
