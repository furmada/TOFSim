...
class Visualizations: 
    ...
    @staticmethod
    def colormesh_3d(sim, size=(10, 10), color_norm="auto", resolution="auto", graph_ax=None, color_legend=True):
        """
        ...
        Parameters:
         ...
         - color_legend: draw a legend entry showing the color range specified.
        ...
        if color_norm != None:
            ...
            plot = ax.scatter(x, y, z, c=values, marker="p", cmap=custom_cmap, vmin=-color_norm, vmax=color_norm)
        else:
            plot = ax.scatter(x, y, z, c=values, marker="p", cmap=custom_cmap)
        if color_legend:
            plt.colorbar(plot)
    ...
    @staticmethod
    def color_xsections_3d(sim3d, ax_loc, size=(10, 10), color_norm="auto", resolution="auto", graph_ax=None, color_legend=True):
        """
        ...
        dummy_sim.V = graph_V
        Visualizations.colormesh_3d(dummy_sim, size, color_norm, resolution, graph_ax, color_legend)
        
    @staticmethod
    def colormesh_2d(sim, size=(10, 10), color_norm="auto", graph_ax=None, color_legend=True):
        """
        Show the potential sim.V as a shaded color-coded map, where negative velocities are red
        and positive velocitues are blue.
        Parameters:
        ...
        if color_norm != None:
            if color_norm == "auto":
                flat_V = sim.V.flatten()
                color_norm = abs(max(abs(max(flat_V)), abs(min(flat_V))))
            plot = ax.pcolormesh(x, y, sim.V.T, cmap=V_COLORS, shading="auto", vmin=-color_norm, vmax=color_norm)
        else:
            plot = ax.pcolormesh(x, y, sim.V.T, cmap=V_COLORS, shading="auto")
        if color_legend:
            plt.colorbar(plot)
...
class VideoMaker(object):
    def __init__(self, figure, axes, videoDir=None, framerate=1):
        ...
        if os.path.exists(self.videoDir):
            frames = os.listdir(self.videoDir)
            for f in frames:
                os.remove(os.path.join(self.videoDir, f))
        else:
    ...
    def make_movie(self, folder="", name="movie.mp4"):
        ...
        os.chdir(cwd)
        if folder != "": os.system("cp {} {}".format(os.path.join(self.videoDir, name), folder))
